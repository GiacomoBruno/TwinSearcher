 ...
 * 2021)
 *
 * @brief BS::thread_pool_light: a fast, lightweight, and easy-to-use C++17
 * thread pool library. This header file contains a light version of the main
 * library, for use when advanced features are not needed.
 */
...
/**
 * @brief A fast, lightweight, and easy-to-use C++17 thread pool class. This is
 * a lighter version of the main thread pool class.
 */
class [[nodiscard]] thread_pool_light
{
   public:
     ...
     *
     * @param thread_count_ The number of threads to use. The default value is
     * the total number of hardware threads available, as reported by the
     ...
     * submitting each block separately to the queue. The user must use
     * wait_for_tasks() or some other method to ensure that the loop finishes
     ...
     * be a signed or unsigned integer. If T1 is not the same as T2, a common
     * type will be automatically inferred.
     ...
     * index after the last index in the block. loop(start, end) should
     * typically involve a loop of the form "for (T i = start; i < end; ++i)".
     * @param num_blocks The maximum number of blocks to split the loop into.
     * The default is to use the number of threads in the pool.
     */
    ...
    void push_loop(T1 first_index_, T2 index_after_last_, F &&loop, size_t num_blocks = 0)
    {
        ...
        if (total_size > 0)
        {
            for (size_t i = 0; i < num_blocks; ++i)
                push_task(std::forward<F>(loop),
                          static_cast<T>(i * block_size) + first_index,
                          (i == num_blocks - 1) ? index_after_last : (static_cast<T>((i + 1) * block_size) + first_index));
        }
    }
     ...
     * submitting each block separately to the queue. The user must use
     * wait_for_tasks() or some other method to ensure that the loop finishes
     * executing, otherwise bad things will happen. This overload is used for
     * the special case where the first index is 0.
     *
     * @tparam F The type of the function to loop through.
     * @tparam T The type of the loop indices. Should be a signed or unsigned
     * integer.
     * @param index_after_last The index after the last index in the loop. The
     * loop will iterate from 0 to (index_after_last - 1) inclusive. In other
     * words, it will be equivalent to "for (T i = 0; i < index_after_last;
     * ++i)". Note that if index_after_last == 0, no blocks will be submitted.
     * @param loop The function to loop through. Will be called once per block.
     * Should take exactly two arguments: the first index in the block and the
     * index after the last index in the block. loop(start, end) should
     * typically involve a loop of the form "for (T i = start; i < end; ++i)".
     ...
     * If the function has a return value, get a future for the eventual
     * returned value. If the function has no return value, get an
     * std::future<void> which can be used to wait until the task finishes.
     *
     * @tparam F The type of the function.
     * @tparam A The types of the zero or more arguments to pass to the
     * function.
   ...
   private:
     ...
     *
     * @param thread_count_ The parameter passed to the constructor. If the
     * parameter is a positive number, then the pool will be created with this
     * number of threads. If the parameter is non-positive, or a parameter was
     * not supplied (in which case it will have the default value of 0), then
     * the pool will be created with the total number of hardware threads
     * available, as obtained from std::thread::hardware_concurrency(). If the
     * latter returns a non-positive number for some reason, then the pool will
     * be created with just one thread.
     ...
     * finishes, the worker notifies wait_for_tasks() in case it is waiting.
     */
    void worker()
    {
        while (running)
    ...
    /**
     * @brief A condition variable used to notify wait_for_tasks() that a tasks
     * is done.
     */
    ...
    /**
     * @brief An atomic variable indicating that wait_for_tasks() is active and
     * expects to be notified whenever a task is done.
     */
