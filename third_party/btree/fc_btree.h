...
template <typename T>
concept DiskAllocable = std::is_same_v<std::remove_cvref_t<T>, T> && std::is_trivially_copyable_v<T> &&(sizeof(T) % alignof(T) == 0);

using attr_t = std::int32_t;

template <Containable K, Containable V>
struct BTreePair
{
    K first;
    V second;

    operator std::pair<const K &, V &>() noexcept
    {
        return { first, second };
    }
...
struct TreePairRef<BTreePair<T, U>>
{
...
template <typename T, typename U>
bool operator==(const PairRefType<BTreePair<T, U>> &lhs, const BTreePair<T, U> &rhs) noexcept
{
    return rhs == lhs;
}
...
struct Projection
{
    const auto &operator()(const V &value) const noexcept
    {
        return value.first;
    }
};
...
{
    auto &operator()(V &iter_ref) noexcept
    {
...
template <Containable K, typename V, attr_t Fanout, typename Comp, bool AllowDup, template <typename T> class Alloc>
requires(Fanout >= 2) class BTreeBase;

template <Containable K, typename V, attr_t Fanout, typename Comp, bool AllowDup, template <typename T> class AllocTemplate>
...
template <Containable K, typename V, attr_t Fanout, typename Comp, bool AllowDup, template <typename T> class AllocTemplate, typename T>
BTreeBase<K, V, Fanout, Comp, AllowDup, AllocTemplate> join(
    BTreeBase<K, V, Fanout, Comp, AllowDup, AllocTemplate> &&tree_left,
    T                                                      &&raw_value,
    BTreeBase<K, V, Fanout, Comp, AllowDup, AllocTemplate> &&tree_right) requires std::is_constructible_v<V, std::remove_cvref_t<T>>;

template <Containable K, typename V, attr_t Fanout, typename Comp, bool AllowDup, template <typename T> class AllocTemplate, typename T>
std::pair<BTreeBase<K, V, Fanout, Comp, AllowDup, AllocTemplate>, BTreeBase<K, V, Fanout, Comp, AllowDup, AllocTemplate>> split(
...
requires(Fanout >= 2) class BTreeBase
{
    ...
    static constexpr bool CompIsLess    = std::is_same_v<Comp, std::ranges::less> || std::is_same_v<Comp, std::less<K>>;
    static constexpr bool CompIsGreater = std::is_same_v<Comp, std::ranges::greater> || std::is_same_v<Comp, std::greater<K>>;

    static constexpr bool use_simd_ =
#if FC_USE_SIMD
        is_set_ && CanUseSimd<K> && (Fanout % (sizeof(K) == 4 ? 8 : 4) == 0) && (Fanout <= 128) && (CompIsLess || CompIsGreater);
#else
        false;
#endif // FC_USE_SIMD
    ...
    {
#endif // FC_USE_SIND
        using keys_type = std::conditional_t<is_disk_, std::array<V, disk_max_nkeys>, std::vector<V>>;
        ...
        attr_t    index_    = 0;
        attr_t    height_   = 0;
        attr_t    num_keys_ = 0; // number of keys in this node, used only for disk variant
        std::vector<std::conditional_t<is_disk_, std::unique_ptr<Node, Deleter>, std::unique_ptr<Node>>> children_;

        Node()
        {
            keys_.reserve(disk_max_nkeys);
        }
    ...
    struct BTreeIterator
    {
        ...
        BTreeIterator(const BTreeIterator<IterTraitsOther> &other) noexcept : BTreeIterator(other.node_, other.index_)
        {
        }
        ...
        friend bool operator==(const BTreeIterator &x, const BTreeIterator &y) noexcept
        {
   ...
   public:
    ...
    using nodeptr_type         = std::conditional_t<DiskAllocable<V>, std::unique_ptr<Node, deleter_type>, std::unique_ptr<Node>>;
    using Proj                 = std::conditional_t<is_set_, std::identity, Projection<const V &>>;
    using ProjIter             = std::conditional_t<is_set_, std::identity, ProjectionIter<PairRefType<V>>>;

    static_assert(std::indirect_strict_weak_order<Comp, std::projected<std::ranges::iterator_t<std::vector<V>>, Proj>>);
   ...
   public:
    using iterator_type               = BTreeIterator<std::conditional_t<is_set_, BTreeConstIterTraits, BTreeRefIterTraits>>;
   ...
   public:
    BTreeBase(const Alloc &alloc = Alloc{}) : alloc_{ alloc }, root_(make_node()), begin_{ root_.get(), 0 }
    {
    }

    BTreeBase(std::initializer_list<value_type> init, const Alloc &alloc = Alloc{}) : BTreeBase(alloc)
    {
        for (auto val : init)
        {
            insert(std::move(val));
    ...
    bool verify(const Node *node) const
    {
        ...
        // invariant: except root, t - 1 <= #(key) <= 2 * t - 1
        assert(!node->parent_ || (node->nkeys() >= Fanout - 1 && node->nkeys() <= 2 * Fanout - 1));

        // invariant: keys are sorted
        assert(std::ranges::is_sorted(node->keys_.begin(), node->keys_.begin() + node->nkeys(), Comp{}, Proj{}));

        // invariant: for internal nodes, t <= #(child) == (#(key) + 1)) <= 2 * t
        assert(
            !node->parent_ || node->is_leaf() ||
            (std::ssize(node->children_) >= Fanout && std::ssize(node->children_) == node->nkeys() + 1 && std::ssize(node->children_) <= 2 * Fanout));

        // index check
        assert(!node->parent_ || node == node->parent_->children_[node->index_].get());
        ...
        if (!node->is_leaf())
        {
            ...
            for (attr_t i = 0; i < node->nkeys(); ++i)
            {
                ...
                assert(!Comp{}(Proj{}(node->keys_[i]), Proj{}(node->children_[i]->keys_[node->children_[i]->nkeys() - 1])));
                assert(!Comp{}(Proj{}(node->children_[i + 1]->keys_[0]), Proj{}(node->keys_[i])));
                // parent check
    ...
    [[nodiscard]] attr_t height() const noexcept
    {
   ...
   protected:
    ...
    void left_rotate(Node *node)
    {
        ...
        assert(node && parent && parent->children_[node->index_].get() == node && node->index_ + 1 < std::ssize(parent->children_) &&
        ...
        if constexpr (is_disk_)
        {
            ...
            std::memmove(sibling->keys_.data(), sibling->keys_.data() + 1, (sibling->num_keys_ - 1) * sizeof(V));
            sibling->num_keys_--;
            if constexpr (use_simd_)
            {
        ...
        if (!node->is_leaf())
        {
            ...
            sibling->children_.front()->index_  = static_cast<attr_t>(std::ssize(node->children_));
    ...
    void left_rotate_n(Node *node, attr_t n)
    {
        ...
        if constexpr (is_disk_)
        {
            ...
            std::memcpy(node->keys_.data() + node->num_keys_, sibling->keys_.data(), (n - 1) * sizeof(V));
            // parent brings one key from sibling
            parent->keys_[node->index_] = sibling->keys_[n - 1];
            std::memmove(sibling->keys_.data(), sibling->keys_.data() + n, (sibling->num_keys_ - n) * sizeof(V));
            sibling->num_keys_ -= n;
            if constexpr (use_simd_)
            {
                for (attr_t k = 0; k < n; ++k)
                {
                    sibling->keys_[sibling->num_keys_ + k] = std::numeric_limits<K>::max();
                }
            }
        }
        else
        {
            ...
            node->keys_.push_back(std::move(parent->keys_[node->index_]));
            // brings n - 1 keys from sibling
            std::ranges::move(sibling->keys_ | std::views::take(n - 1), std::back_inserter(node->keys_));
            // parent brings one key from sibling
        ...
        if (!node->is_leaf())
        {
            ...
            std::ranges::move(sibling->children_ | std::views::take(n), std::back_inserter(node->children_));
    ...
    {
        auto parent = node->parent_;
        assert(node && parent && parent->children_[node->index_].get() == node && node->index_ - 1 >= 0 &&
               parent->children_[node->index_ - 1]->can_take_key());
        auto sibling = parent->children_[node->index_ - 1].get();

        if constexpr (is_disk_)
        {
            std::memmove(node->keys_.data() + 1, node->keys_.data(), node->num_keys_ * sizeof(V));
            node->num_keys_++;
        ...
        {
            node->keys_.insert(node->keys_.begin(), std::move(parent->keys_[node->index_ - 1]));
        ...
        if (!node->is_leaf())
        {
            ...
            node->children_.insert(node->children_.begin(), std::move(sibling->children_.back()));
            sibling->children_.pop_back();
    ...
    void right_rotate_n(Node *node, attr_t n)
    {
        ...
        if constexpr (is_disk_)
        {
            std::memcpy(node->keys_.data() + node->num_keys_, sibling->keys_.data() + (sibling->num_keys_ - n), (n - 1) * sizeof(V));
            node->num_keys_ += (n - 1);
            node->keys_[node->num_keys_] = parent->keys_[node->index_ - 1];
            node->num_keys_++;
            parent->keys_[node->index_ - 1] = sibling->keys_[sibling->num_keys_ - 1];

            std::rotate(std::make_reverse_iterator(node->keys_.begin() + node->num_keys_),
        ...
        {
            // brings n - 1 keys from sibling
            std::ranges::move(sibling->keys_ | std::views::drop(sibling->nkeys() - n) | std::views::take(n - 1), std::back_inserter(node->keys_));
            // brings one key from parent
            node->keys_.push_back(std::move(parent->keys_[node->index_ - 1]));
            // parent brings one key from sibling
            parent->keys_[node->index_ - 1] = std::move(sibling->keys_.back());
            // right rotate n
            std::ranges::rotate(node->keys_ | std::views::reverse, node->keys_.rbegin() + n);
            sibling->keys_.resize(sibling->nkeys() - n);
        }
        ...
        if (!node->is_leaf())
        {
            ...
            for (auto &&immigrant : sibling->children_ | std::views::drop(std::ssize(sibling->children_) - n))
            {
                immigrant->parent_ = node;
                immigrant->index_  = immigrant_index++;
                orphan_size += immigrant->size_;
            ...
            std::ranges::move(sibling->children_ | std::views::drop(std::ssize(sibling->children_) - n), std::back_inserter(node->children_));
            std::ranges::rotate(node->children_ | std::views::reverse, node->children_.rbegin() + n);
            for (attr_t idx = 0; idx < n; ++idx)
            {
                sibling->children_.pop_back();
            }
    ...
    auto get_lb(const K &key, const Node *x) const noexcept
    {
        if constexpr (use_simd_)
        {
            return get_lb_simd<K, CompIsLess>(key, x->keys_.data(), x->keys_.data() + 2 * Fanout);
        }
        else if constexpr (use_linsearch_)
        {
            auto lbcomp = [&key](const K &other) { return Comp{}(other, key); };
            return std::distance(x->keys_.begin(), std::ranges::find_if_not(x->keys_.begin(), x->keys_.begin() + x->nkeys(), lbcomp, Proj{}));
        }
        else
        {
            return std::distance(x->keys_.begin(), std::ranges::lower_bound(x->keys_.begin(), x->keys_.begin() + x->nkeys(), key, Comp{}, Proj{}));
        }
    }
    ...
    auto get_ub(const K &key, const Node *x) const noexcept
    {
        if constexpr (use_simd_)
        {
            return get_ub_simd<K, CompIsLess>(key, x->keys_.data(), x->keys_.data() + 2 * Fanout);
        }
        else if constexpr (use_linsearch_)
        {
            auto ubcomp = [&key](const K &other) { return Comp{}(key, other); };
            return std::distance(x->keys_.begin(), std::ranges::find_if(x->keys_.begin(), x->keys_.begin() + x->nkeys(), ubcomp, Proj{}));
        }
        else
        {
            return std::distance(x->keys_.begin(), std::ranges::upper_bound(x->keys_.begin(), x->keys_.begin() + x->nkeys(), key, Comp{}, Proj{}));
        }
    }
    ...
    void split_child(Node *y)
    {
        ...
        if constexpr (is_disk_)
        {
            std::memcpy(z->keys_.data(), y->keys_.data() + Fanout, (y->num_keys_ - Fanout) * sizeof(V));
            z->num_keys_ = y->num_keys_ - Fanout;
        }
        else
        {
            std::ranges::move(y->keys_ | std::views::drop(Fanout), std::back_inserter(z->keys_));
        }
        auto z_size = z->nkeys();
        if (!y->is_leaf())
        {
            ...
            std::ranges::move(y->children_ | std::views::drop(Fanout), std::back_inserter(z->children_));
        ...
        if constexpr (is_disk_)
        {
            std::memmove(x->keys_.data() + i + 1, x->keys_.data() + i, (x->num_keys_ - i) * sizeof(V));
            x->num_keys_++;
    ...
    void merge_child(Node *y)
    {
        ...
        auto  i = y->index_;
        Node *x = y->parent_;
        assert(x && y == x->children_[i].get() && !x->is_leaf() && i >= 0 && i + 1 < std::ssize(x->children_));
        ...
        if constexpr (is_disk_)
        {
            ...
            y->num_keys_++;
            std::memcpy(y->keys_.data() + y->num_keys_, sibling->keys_.data(), sibling->num_keys_ * sizeof(V));
        ...
        if constexpr (is_disk_)
        {
            std::memmove(x->keys_.data() + i, x->keys_.data() + i + 1, (x->num_keys_ - (i + 1)) * sizeof(V));
            x->num_keys_--;
            if constexpr (use_simd_)
            {
    ...
    template <typename T>
    iterator_type insert_leaf(Node *node, attr_t i, T &&value) requires std::is_same_v<std::remove_cvref_t<T>, V>
    {
        ...
        if constexpr (is_disk_)
        {
            std::memmove(node->keys_.data() + i + 1, node->keys_.data() + i, (node->num_keys_ - i) * sizeof(V));
    ...
    template <typename T>
    iterator_type insert_ub(T &&key) requires(AllowDup &&std::is_same_v<std::remove_cvref_t<T>, V>)
    {
        auto x = root_.get();
        while (true)
        {
    ...
    template <typename T>
    std::pair<iterator_type, bool> insert_lb(T &&key) requires(!AllowDup && std::is_same_v<std::remove_cvref_t<T>, V>)
    {
        auto x = root_.get();
        while (true)
        {
            ...
            {
                return { insert_leaf(x, static_cast<attr_t>(i), std::forward<T>(key)), true };
            }
            else
            {
                if (x->children_[i]->is_full())
    ...
    {
        assert(node && i >= 0 && i < node->nkeys() && node->is_leaf() && !node->has_minimal_keys());
        bool update_begin = (begin_ == const_iterator_type(node, i));
        if constexpr (is_disk_)
        {
            std::memmove(node->keys_.data() + i, node->keys_.data() + i + 1, (node->num_keys_ - (i + 1)) * sizeof(V));
            node->num_keys_--;
            if constexpr (use_simd_)
            {
    ...
    size_t erase_lb(Node *x, const K &key) requires(!AllowDup)
    {
        while (true)
        {
            ...
            else
            {
                ...
                {
                    if (i + 1 < std::ssize(x->children_) && x->children_[i + 1]->can_take_key())
                    {
                        left_rotate(next);
                    }
    ...
    iterator_type erase_hint([[maybe_unused]] const V &value, std::vector<attr_t> &hints)
    {
        auto x = root_.get();
        while (true)
        {
            ...
            if (hints.empty())
            {
                ...
                else if (x->children_[i]->can_take_key())
                {
                    ...
                    while (!curr->is_leaf())
                    {
                        hints.push_back(static_cast<attr_t>(std::ssize(curr->children_)) - 1);
   ...
   public:
    ...
    {
        return { iterator_type(find_lower_bound(key)), iterator_type(find_upper_bound(key)) };
    }

    std::ranges::subrange<const_iterator_type> equal_range(const K &key) const
    {
        return { const_iterator_type(find_lower_bound(key)), const_iterator_type(find_upper_bound(key)) };
    }

    std::ranges::subrange<const_iterator_type> enumerate(const K &a, const K &b) const
    {
        if (Comp{}(b, a))
        {
            throw std::invalid_argument("b < a in enumerate()");
        }
        return { const_iterator_type(find_lower_bound(a)), const_iterator_type(find_upper_bound(b)) };
    }
   ...
   protected:
    template <typename T>
    std::conditional_t<AllowDup, iterator_type, std::pair<iterator_type, bool>> insert_value(T &&key) requires(
   ...
   public:
    std::conditional_t<AllowDup, iterator_type, std::pair<iterator_type, bool>> insert(const V &key)
    {
        return insert_value(key);
    }

    std::conditional_t<AllowDup, iterator_type, std::pair<iterator_type, bool>> insert(V &&key)
    {
        return insert_value(std::move(key));
    }

    template <typename... Args>
    std::conditional_t<AllowDup, iterator_type, std::pair<iterator_type, bool>> emplace(Args &&...args) requires std::is_constructible_v<V, Args...>
    {
    ...
    requires std::is_constructible_v<V, std::iter_reference_t<Iter>> size_type insert_range(Iter first, Iter last)
    {
        auto [min_elem, max_elem] = std::ranges::minmax_element(first, last, Comp{}, Proj{});
    ...
    requires std::is_constructible_v<V, std::ranges::range_reference_t<R>> size_type insert_range(R &&r)
    {
    ...
    size_type erase(const K &key)
    {
        if constexpr (AllowDup)
        {
            return erase_range(const_iterator_type(find_lower_bound(key, false)), const_iterator_type(find_upper_bound(key, false)));
        }
        else
        {
    ...
    size_type erase_range(const K &a, const K &b)
    {
        ...
        return erase_range(const_iterator_type(find_lower_bound(a)), const_iterator_type(find_upper_bound(b)));
    }
   ...
   private:
    ...
    // attack
    static constexpr std::size_t max_possible_height = (44UL - std::bit_width(static_cast<std::size_t>(2 * Fanout))) / std::bit_width(keydata_size);

   public:
    friend std::istream &operator>>(std::istream &is, BTreeBase &tree) requires(is_disk_)
    {
   ...
   protected:
    ...
    bool deserialize_node(std::istream &is, Node *node, attr_t node_index, attr_t node_height) requires(is_disk_)
    {
        assert(node);
        ...
        if (node->num_keys_ >= 2 * Fanout || (node != root_.get() && node->num_keys_ < Fanout - 1) || node->num_keys_ < 0)
        {
            std::cerr << "Tree deserialization: nkeys is invalid\n";
            is.clear(std::ios_base::failbit);
            return false;
        }
        if (!is.read(reinterpret_cast<char *>(node->keys_.data()), static_cast<std::size_t>(node->num_keys_) * sizeof(V)))
        {
        ...
        if (node_height > 0)
        {
            for (attr_t i = 0; i <= node->num_keys_; ++i)
            {
                ...
                if (!deserialize_node(is, node->children_[i].get(), i, node_height - 1))
                {
                    return false;
                }
            }
   ...
   public:
    friend std::ostream &operator<<(std::ostream &os, const BTreeBase &tree) requires(is_disk_)
    {
        std::uint64_t tree_code = begin_code;
        if (!os.write(reinterpret_cast<char *>(&tree_code), sizeof(std::uint64_t)))
        {
   ...
   protected:
    // preorder DFS traversal
    bool serialize_node(std::ostream &os, const Node *node) const requires(is_disk_)
    {
        assert(node);
        if (!os.write(reinterpret_cast<const char *>(&node->num_keys_), sizeof(attr_t)))
        {
            std::cerr << "Tree serialization: nkeys write error\n";
            return false;
        }
        if (!os.write(reinterpret_cast<const char *>(node->keys_.data()), static_cast<std::size_t>(node->num_keys_) * sizeof(V)))
        {
   ...
   public:
    template <Containable K_, typename V_, attr_t Fanout_, typename Comp_, bool AllowDup_, template <typename T> class AllocTemplate_>
    friend BTreeBase<K_, V_, Fanout_, Comp_, AllowDup_, AllocTemplate_> join(
        BTreeBase<K_, V_, Fanout_, Comp_, AllowDup_, AllocTemplate_> &&tree_left,
        BTreeBase<K_, V_, Fanout_, Comp_, AllowDup_, AllocTemplate_> &&tree_right);

    template <Containable K_, typename V_, attr_t Fanout_, typename Comp_, bool AllowDup_, template <typename T> class AllocTemplate_, typename T_>
    friend BTreeBase<K_, V_, Fanout_, Comp_, AllowDup_, AllocTemplate_> join(
        BTreeBase<K_, V_, Fanout_, Comp_, AllowDup_, AllocTemplate_> &&tree_left,
        T_                                                           &&raw_value,
        BTreeBase<K_, V_, Fanout_, Comp_, AllowDup_, AllocTemplate_> &&tree_right) requires std::is_constructible_v<V_, std::remove_cvref_t<T_>>;

   protected:
    std::pair<BTreeBase, BTreeBase> split_to_two_trees(const_iterator_type iter_lb, const_iterator_type iter_ub)
    {
        ...
        while (!lindices.empty())
        {
            ...
            if (xl->is_leaf())
            {
                ...
                if (il > 0)
                {
                    ...
                    else
                    {
                        ...
                        std::ranges::move(xl->keys_ | std::views::take(il), std::back_inserter(lroot->keys_));
                    }
                    lroot->size_ += il;
                }
            ...
            else
            {
                if (il > 0)
                {
                    ...
                    if constexpr (is_disk_)
                    {
                        std::memcpy(slroot->keys_.data(), xl->keys_.data(), (il - 1) * sizeof(V));
                        slroot->num_keys_ += (il - 1);
                    }
                    else
                    {
                        std::ranges::move(xl->keys_ | std::views::take(il - 1), std::back_inserter(slroot->keys_));
                    }
                    slroot->size_ += (il - 1);

                    slroot->children_.reserve(Fanout * 2);

                    std::ranges::move(xl->children_ | std::views::take(il), std::back_inserter(slroot->children_));
                    ...
                    BTreeBase new_tree_left = join(std::move(supertree_left), std::move(xl->keys_[il - 1]), std::move(tree_left));
        ...
        while (!rindices.empty())
        {
            ...
            if (xr->is_leaf())
            {
                ...
                if (ir < xr->nkeys())
                {
                    ...
                    if constexpr (is_disk_)
                    {
                        std::memcpy(rroot->keys_.data(), xr->keys_.data() + ir, immigrants * sizeof(V));
                        rroot->num_keys_ += immigrants;
                    }
                    else
                    {
                        // send right n - (i + 1) keys to rroot
                        std::ranges::move(xr->keys_ | std::views::drop(ir), std::back_inserter(rroot->keys_));
                    }
            ...
            else
            {
                if (ir + 1 < std::ssize(xr->children_))
                {
                    ...
                    if constexpr (is_disk_)
                    {
                        std::memcpy(srroot->keys_.data(), xr->keys_.data() + (ir + 1), immigrants * sizeof(V));
                        srroot->num_keys_ += immigrants;
                    }
                    else
                    {
                        std::ranges::move(xr->keys_ | std::views::drop(ir + 1), std::back_inserter(srroot->keys_));
                    }
                    srroot->size_ += immigrants;

                    srroot->children_.reserve(Fanout * 2);

                    std::ranges::move(xr->children_ | std::views::drop(ir + 1), std::back_inserter(srroot->children_));
                    ...
                    BTreeBase new_tree_right = join(std::move(tree_right), std::move(xr->keys_[ir]), std::move(supertree_right));
   ...
   public:
    template <Containable K_, typename V_, attr_t Fanout_, typename Comp_, bool AllowDup_, template <typename T> class AllocTemplate_, typename T>
    friend std::pair<BTreeBase<K_, V_, Fanout_, Comp_, AllowDup_, AllocTemplate_>, BTreeBase<K_, V_, Fanout_, Comp_, AllowDup_, AllocTemplate_>>
    ...
    friend std::pair<BTreeBase<K_, V_, Fanout_, Comp_, AllowDup_, AllocTemplate_>, BTreeBase<K_, V_, Fanout_, Comp_, AllowDup_, AllocTemplate_>>
        split(BTreeBase<K_, V_, Fanout_, Comp_, AllowDup_, AllocTemplate_> &&tree,
              T                                                            &&raw_key1,
              T                                                            &&raw_key2) requires std::is_constructible_v<K_, std::remove_cvref_t<T>>;
};
                                                            ...
                                                            BTreeBase<K, V, Fanout, Comp, AllowDup, AllocTemplate> &&tree_right)
{
    ...
    else
    {
        ...
        V    mid_value = *it;
        tree_right.erase(it);
        return join(std::move(tree_left), std::move(mid_value), std::move(tree_right));
    }
}

template <Containable K, typename V, attr_t Fanout, typename Comp, bool AllowDup, template <typename T> class AllocTemplate, typename T_>
BTreeBase<K, V, Fanout, Comp, AllowDup, AllocTemplate> join(
    BTreeBase<K, V, Fanout, Comp, AllowDup, AllocTemplate> &&tree_left,
    T_                                                     &&raw_value,
    BTreeBase<K, V, Fanout, Comp, AllowDup, AllocTemplate> &&tree_right) requires std::is_constructible_v<V, std::remove_cvref_t<T_>>
{
    ...
    if ((!tree_left.empty() && Comp{}(Proj{}(mid_value), Proj{}(*tree_left.crbegin()))) ||
        (!tree_right.empty() && Comp{}(Proj{}(*tree_right.cbegin()), Proj{}(mid_value))))
    {
    ...
    if (height_left >= height_right)
    {
        ...
        else
        {
            ...
            tree_right.root_->index_  = static_cast<attr_t>(std::ssize(parent->children_));
    ...
    else
    {
        ...
        if constexpr (is_disk_)
        {
            std::memmove(parent->keys_.data() + 1, parent->keys_.data(), parent->num_keys_ * sizeof(V));
        ...
        parent->children_.insert(parent->children_.begin(), std::move(tree_left.root_));
    ...
    T                                                      &&raw_key) requires(std::is_constructible_v<K, std::remove_cvref_t<T>>)
{
    ...
    return tree.split_to_two_trees(tree.find_lower_bound(mid_key, false), tree.find_upper_bound(mid_key, false));
}
    ...
    T                                                      &&raw_key2) requires(std::is_constructible_v<K, std::remove_cvref_t<T>>)
{
    ...
    return tree.split_to_two_trees(tree.find_lower_bound(key1, false), tree.find_upper_bound(key2, false));
}

template <Containable K, attr_t t = 64, typename Comp = std::ranges::less, template <typename T> class AllocTemplate = std::allocator>
using BTreeSet = BTreeBase<K, K, t, Comp, false, AllocTemplate>;

template <Containable K, attr_t t = 64, typename Comp = std::ranges::less, template <typename T> class AllocTemplate = std::allocator>
using BTreeMultiSet = BTreeBase<K, K, t, Comp, true, AllocTemplate>;

template <Containable K, Containable V, attr_t t = 64, typename Comp = std::ranges::less, template <typename T> class AllocTemplate = std::allocator>
using BTreeMap = BTreeBase<K, BTreePair<K, V>, t, Comp, false, AllocTemplate>;

template <Containable K, Containable V, attr_t t = 64, typename Comp = std::ranges::less, template <typename T> class AllocTemplate = std::allocator>
using BTreeMultiMap = BTreeBase<K, BTreePair<K, V>, t, Comp, true, AllocTemplate>;
