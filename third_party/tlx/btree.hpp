...
class BTree
{
   ...
   public:
    ...
    //! comparison function.
    explicit BTree(const allocator_type &alloc = allocator_type()) : root_(nullptr), head_leaf_(nullptr), tail_leaf_(nullptr), allocator_(alloc)
    {
    }
   ...
   private:
    ...
    int compare(const key_type &a, const key_type &b) const
    {
   ...
   private:
    ...
    unsigned short find_lower(const node_type *n, const key_type &key) const
    {
        if (sizeof(*n) > traits::binsearch_threshold)
        {
            ...
            {
                unsigned short i = 0;
                while (i < n->slotuse && compare(n->key(i), key) < 0)
                    ++i;
        ...
        {
            unsigned short lo = 0;
            while (lo < n->slotuse && compare(n->key(lo), key) < 0)
                ++lo;
            return lo;
        }
    }
   ...
   public:
    ...
    iterator find(const key_type &key)
    {
        ...
        unsigned short slot = find_lower(leaf, key);
        return (slot < leaf->slotuse && compare(key, leaf->key(slot)) == 0) ? iterator(leaf, slot) : end();
    }
...
#    endif
        ...
        else // n->is_leafnode() == true
        {
            ...
            if (!allow_duplicates && slot < leaf->slotuse && compare(key, leaf->key(slot)) == 0)
            {
   ...
   public:
    ...
    void bulk_load(Iterator ibegin, Iterator iend)
    {
        ...
        TLX_BTREE_PRINT("BTree::bulk_load, level 0: " << stats_.size << " items into " << num_leaves << " leaves with up to "
        ...
        TLX_BTREE_PRINT("BTree::bulk_load, level 1: " << num_leaves << " leaves in " << num_parents << " inner nodes with up to "
        ...
        for (int level = 2; num_parents != 1; ++level)
        {
            ...
            TLX_BTREE_PRINT("BTree::bulk_load, level " << level << ": " << num_children << " children in " << num_parents
   ...
   private:
                               ...
                               unsigned int    parentslot)
    {
        ...
        else // !curr->is_leafnode()
        {
            ...
            result_t result = erase_one_descend(key, inner->childid[slot], myleft, myright, myleft_parent, myright_parent, inner, slot);
                                ...
                                unsigned int    parentslot)
    {
        ...
        else // !curr->is_leafnode()
        {
            ...
            while (slot <= inner->slotuse)
            {
                ...
                result = erase_iter_descend(iter, inner->childid[slot], myleft, myright, myleft_parent, myright_parent, inner, slot);
    ...
    static void shift_right_inner(InnerNode *left, InnerNode *right, InnerNode *parent, unsigned int parentslot)
    {
        ...
        std::copy_backward(right->childid, right->childid + right->slotuse + 1, right->childid + right->slotuse + 1 + shiftnum);
   ...
   private:
    ...
    void verify_node(const node *n, key_type *minkey, key_type *maxkey, tree_stats &vstats) const
    {
        ...
        if (n->is_leafnode())
        {
            ...
            {
                tlx_die_unless(compare(leaf->key(slot), leaf->key(slot + 1)) <= 0);
            }
        ...
        else // !n->is_leafnode()
        {
            ...
            {
                tlx_die_unless(compare(inner->key(slot), inner->key(slot + 1)) <= 0);
            }
            ...
            for (unsigned short slot = 0; slot <= inner->slotuse; ++slot)
            {
                ...
                if (slot == 0)
                    *minkey = subminkey;
                    ...
                    tlx_die_unless(compare(subminkey, inner->key(slot - 1)) >= 0);

                if (slot == inner->slotuse)
                    *maxkey = submaxkey;
                else
                    tlx_die_unless(compare(inner->key(slot), submaxkey) == 0);
    ...
    void verify_leaflinks() const
    {
        ...
        while (n)
        {
            ...
            {
                tlx_die_unless(compare(n->key(slot), n->key(slot + 1)) <= 0);
            }
            ...
            if (n->next_leaf)
            {
                tlx_die_unless(compare(n->key(n->slotuse - 1) <= 0, n->next_leaf->key(0)));
