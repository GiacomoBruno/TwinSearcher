...
private:
    ...
    unsigned short find_lower(const node_type* n, const key_type& key) const {
        if (sizeof(*n) > traits::binsearch_threshold)
        {
            ...
            {
                unsigned short i = 0;
                while (i < n->slotuse && compare(n->key(i), key) < 0) ++i;
        ...
        {
            unsigned short lo = 0;
            while (lo < n->slotuse && compare(n->key(lo), key) < 0) ++lo;
            return lo;
        }
    }
...
public:
    ...
    iterator find(const key_type& key) {
        ...
        unsigned short slot = find_lower(leaf, key);
        return (slot < leaf->slotuse && compare(key, leaf->key(slot)) == 0)
    ...
    */
    std::pair<iterator, bool> insert_descend(
        ...
        else // n->is_leafnode() == true
        {
            ...
            if (!allow_duplicates &&
                slot < leaf->slotuse && compare(key, leaf->key(slot)) == 0) {
...
private:
    ...
    void verify_node(const node* n, key_type* minkey, key_type* maxkey,
        ...
        if (n->is_leafnode())
        {
            ...
            {
                tlx_die_unless(
                    compare(leaf->key(slot), leaf->key(slot + 1)) <= 0);
            }
        ...
        else // !n->is_leafnode()
        {
            ...
            {
                tlx_die_unless(
                    compare(inner->key(slot), inner->key(slot + 1)) <= 0);
            }
            ...
            for (unsigned short slot = 0; slot <= inner->slotuse; ++slot)
            {
                ...
                if (slot == 0)
                    *minkey = subminkey;
                    ...
                    tlx_die_unless(
                        compare(subminkey, inner->key(slot - 1)) >= 0);
    ...
    void verify_leaflinks() const {
        ...
        while (n)
        {
            ...
            if (n->next_leaf)
            {
                tlx_die_unless(compare(n->key(n->slotuse - 1) <= 0,
                                             n->next_leaf->key(0)));
